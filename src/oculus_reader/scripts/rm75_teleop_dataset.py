#!/usr/bin/env python3
"""Synchronized Meta Quest 3 + RM75 + RealSense data collection.
...
def _vr_action(buttons, previous_buttons):
    ...
    r_now = bool(buttons.get("RJ", False) or buttons.get("A", False))
    # X ends the current episode; A+X ends the whole collection. X+Y remains
    # a gripper toggle and is therefore excluded from the lifecycle edge.
    ...
    if l_now and not l_old:
        # Require an intentional triple-click within two seconds to close the
        # current episode.  A single accidental X press must not terminate or
        # save a trajectory.  Keep the timestamps in the caller-owned state so
        # this works for both the A/X fallback and LJ input.
        now = time.monotonic()
        presses = [t for t in previous_buttons.get("_left_press_times", [])
                   if now - t <= 2.0]
        presses.append(now)
        previous_buttons["_left_press_times"] = presses
        if len(presses) >= 3:
            previous_buttons["_left_press_times"] = []
            return "episode_end"
    return None
...
def _robot_record(controllers, reader=None):
    result = {}
    grip_state = getattr(reader, "_grip_closed", {}) if reader is not None else {}
    for name, controller in controllers.items():
        # Record the commanded gripper value alongside joints.  The Quest
        # button/trigger fields are also stored separately, but they do not
        # tell us which width was actually sent to the RM controller.
        closed = bool(grip_state.get(name, False))
        width = float(getattr(controller, "last_gripper_width",
                              0.0 if closed else controller.gripper_max_width))
        # A+B/X+Y state is the intended digital control; expose it explicitly
        # even when a controller has not yet received a gripper command.
        if not hasattr(controller, "last_gripper_width"):
            width = 0.0 if closed else float(controller.gripper_max_width)
        result[name] = {
            "joints_rad": controller.current_joint_positions(),
            "gripper": width,
    ...
    try:
        ...
        while not STOP:
            ...
            if now >= next_control:
                ...
                if action == "start":
                    if not recording_active:
                        episode_index += 1
                        save_dir = session_root / "episode_{:04d}".format(episode_index)
                        save_dir.mkdir(parents=True, exist_ok=True)
            ...
            if controllers:
                ...
                record["robot"] = _jsonable(_robot_record(controllers, reader))
